...
class RunDetailViewController: UIViewController, MKMapViewDelegate {
    ...
    @IBAction func startRunButtonTapped(_ sender: UIButton) {
        ...
        // Check if the user is already in the first checkpoint.
        if LocationManager.shared.isUserInFirstCheckpoint() {
            // User is in the first checkpoint, trigger the first checkpoint logic directly.
            LocationManager.shared.triggerFirstCheckpointLogic()
            self.startLocationTracking()
            // Start monitoring checkpoints skipping the first one
            ...
            // User is not in the first checkpoint, play the "start.mp3" audio.
